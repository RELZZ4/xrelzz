document.addEventListener('DOMContentLoaded', () => {
    ...
    modalConfirmBtn.addEventListener('click', () => {
        chatHistory = [];
        localStorage.removeItem('painzyAiChatHistory');
    ...
    async function fetchAIResponse(prompt, model, isRetry = false) {
        ...
        switch (model) {
            case 'alicia': url = `https://painzy-api.vercel.app/ai/alicia?text=${encodedPrompt}`; break;
        ...
        try {
            ...
            let aiText = '';
            if (data.message) aiText = data.message;
            ...
            if (!isRetry) {
                await fetchAIResponse(prompt, 'alicia', true);
            } else {
    ...
    async function generateImage(prompt) {
        const nsfwKeywords = ['nsfw', 'telanjang', 'bugil', 'nude', 'naked', 'sex', 'seks', 'porn', 'porno', 'hentai', 'gore', 'darah', 'kekerasan', 'violence', 'sadis', 'explicit', 'r18', '18+'];
    ...
    function renderMessage(message) {
        ...
        if (message.type === 'image') {
            ...
            downloadBtn.download = `painzy-ai-image-${Date.now()}.png`;
    ...
    function saveHistory() {
        localStorage.setItem('painzyAiChatHistory', JSON.stringify(chatHistory));
    }

    function loadHistory() {
        const savedHistory = localStorage.getItem('painzyAiChatHistory');
        chatWindow.innerHTML = '';
        if (savedHistory) {
